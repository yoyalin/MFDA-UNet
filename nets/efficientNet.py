...
def main():
    ...
    """
    # 1. Selecione um nome de modelo da lista de modelos válidos
    # Os modelos disponíveis são: 'efficientnet-b0' a 'efficientnet-b8', e 'efficientnet-l2'
    model_name = 'efficientnet-b0'
    print(f"A validar o modelo: {model_name}\n")

    # 2. Carregue o modelo EfficientNet pré-treinado
    # O modelo é carregado com pesos pré-treinados do ImageNet por padrão
    try:
    ...
    # Isto é importante para camadas como Dropout e BatchNorm se comportarem corretamente para inferência
    model.eval()

    # 4. Obtenha o tamanho de imagem de entrada esperado para o modelo
    # Para 'efficientnet-b0', espera-se que seja 224x224. [2, 9, 10]
    image_size = EfficientNet.get_image_size(model_name)
    print(f"Tamanho de imagem de entrada esperado: ({image_size}, {image_size})\n")

    # 5. Crie um tensor de entrada fictício
    # O tensor representa um lote de 1 imagem, com 3 canais de cor (RGB), e o tamanho de imagem correto. [1]
    # torch.rand cria um tensor com valores aleatórios entre 0 e 1. [1, 4]
    try:
    ...
    # 6. Passe a entrada pelo modelo para validar a passagem para a frente (forward pass)
    with torch.no_grad(): # Desativar o cálculo de gradiente para inferência
        # a) Validar a função forward principal
        try:
            outputs = model(inputs)
            print("Passagem para a frente (forward) principal bem-sucedida.")
            # A saída deve ter a forma (batch_size, num_classes), que é (1, 1000) por padrão. [3]
            print(f"Forma da saída final: {outputs.shape}\n")
        except Exception as e:
            print(f"Erro durante a passagem para a frente (forward) principal: {e}")

        # b) Validar a função extract_features
        try:
            features = model.extract_features(inputs)
            print("Extração de características bem-sucedida.")
            # A forma do mapa de características dependerá da variante do EfficientNet
            print(f"Forma das características extraídas: {features.shape}\n")
        except Exception as e:
            print(f"Erro durante a extração de características: {e}")

        # c) Validar a função extract_endpoints
        try:
            endpoints = model.extract_endpoints(inputs)
